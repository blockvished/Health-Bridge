  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <SettingsSection title="Autopost Settings">
        <MultiSelectDropdown
          label="Autopost Posts to Google Business Profile of this user(s)"
          selectedValue={settings.selectedUsers}
          setSelectedValue={handleSelectChange}
          options={settings.users.map((user) => user.userId)}
          description="Select each of the users that you want to automatically post to Google Business Profile when a new post is published."
        />

        <SingleSelectDropdown
          label="Select GBP Button Type"
          selectedValue={settings.urlShortener}
          setSelectedValue={handleUrlShortenerChange} // Pass handleUrlShortenerChange directly
          options={["Book", "Order Online", "Learn More", "Sign up", "Call"]}
        />
