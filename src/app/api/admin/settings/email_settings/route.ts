  ...
  try {
    ...
    if (!mailType || !mailTitle || !mailHost || !mailPort) {
      return NextResponse.json(
        { 
          success: false, 
          message: 'Missing required fields: mailType, mailTitle, mailHost, mailPort' 
        },
        { status: 400 }
      );
    }
    ...
    if (mailEncryption && !['SSL', 'TLS', 'STARTTLS', ''].includes(mailEncryption)) {
      return NextResponse.json(
        { 
          success: false, 
          message: 'Mail encryption must be SSL, TLS, STARTTLS, or empty for no encryption' 
        },
        { status: 400 }
      );
    }
    ...
    const { mailPassword: _, ...safeResult } = result[0];

    return NextResponse.json({
      success: true,
      message: existingConfig.length > 0 ? 'Email settings updated successfully' : 'Email settings created successfully',
      data: safeResult
    });
...
export async function DELETE() {
  try {
    ...
    if (result.length === 0) {
      return NextResponse.json(
        { success: false, message: 'No email configuration found to deactivate' },
        { status: 404 }
      );
    }
