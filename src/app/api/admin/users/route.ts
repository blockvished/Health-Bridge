  ...
  try {
    ...
    const doctors = await db
      .select({
        ...
        expireAt: doctor.expireAt, // Added expiry date
      })
      .from(doctor)
    ...
    // TODO based on plan expiry
    const expiredAccounts = doctors.filter((doc) => {
      return doc.expireAt && new Date(doc.expireAt) < now;
    }).length;
