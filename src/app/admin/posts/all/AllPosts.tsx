...
function Posts() {
  ...
  const publishedPosts: Post[] = [
    { 
      id: 1, 
      content: 'Comprehensive tools to manage efficient and effective health care practice.', 
      image: 'image1.jpg', 
      status: 'Published', 
      date: '15 Jun 2023',
      interactions: 24,
    ...
    { 
      id: 2, 
      content: 'Our all-in-one healthcare practice management system is designed to simplify and optimize your clinical operations while enhancing your marketing and promotional efforts.', 
      image: 'image2.jpg', 
      status: 'Published', 
      date: '12 Jun 2023',
      interactions: 18,
  ...
  const scheduledPosts: Post[] = [
    { 
      id: 3, 
      content: '5 Clinic Owners Use Doctors Method to Make [Post] increase revenue by 200% in 6 months through…', 
    ...
    { 
      id: 4, 
      content: '3 Red Flags Your Practice is Falling Behind: [Spending hours on WhatsApp]', 
  ...
  const SocialIcons = () => (
    <div className="flex space-x-2">
      <div className="w-6 h-6 rounded-full bg-blue-600 flex items-center justify-center text-white text-xs shadow-sm">f</div>
      <div className="w-6 h-6 rounded-full bg-blue-400 flex items-center justify-center text-white text-xs shadow-sm">t</div>
      <div className="w-6 h-6 rounded-full bg-pink-500 flex items-center justify-center text-white text-xs shadow-sm">i</div>
    </div>
  );
  ...
  const renderEmptyState = (section: string) => (
    <div className="py-12 text-center bg-gray-50 rounded-lg border border-dashed border-gray-300">
      ...
      <h3 className="text-lg font-medium text-gray-700 mb-1">No {section} posts found</h3>
      <p className="text-gray-500 mb-4">Get started by creating your first {section.toLowerCase()} post</p>
            ...
            <table className="w-full table-auto">
              <thead>
                <tr className="border-b-2 border-gray-100 text-sm text-gray-600">
                  <th className="text-left px-4 py-3 font-medium w-1/6">PUBLISHED ON</th>
                  <th className="text-left px-4 py-3 font-medium w-1/2">POST CONTENT</th>
              ...
              <tbody>
                {publishedPosts.map((post: Post) => (
                  <tr key={post.id} className="border-b border-gray-100 hover:bg-gray-50 text-sm">
                    <td className="px-4 py-4 text-gray-800 font-medium">{post.date}</td>
                    ...
                    <td className="px-4 py-4">
                      <div className="flex items-center">
                        {post.publishedByAvatar && renderAvatar(post.publishedByAvatar)}
                        <span className="ml-2 text-gray-800">{post.publishedBy}</span>
                      </div>
                    </td>
                  </tr>
                ))}
                ...
                <div key={post.id} className="bg-white rounded-lg shadow-sm overflow-hidden border border-gray-200">
                  <div className="p-4">
                    ...
                    <div className="flex items-start mb-3">
                      ...
                      <p className="font-medium text-gray-800 line-clamp-3">{post.content}</p>
                    </div>
                    ...
                    <div className="flex justify-between items-center pt-2 border-t border-gray-100">
                      ...
                      <div className="flex items-center">
                        {post.publishedByAvatar && renderAvatar(post.publishedByAvatar)}
                        <span className="ml-2 text-xs text-gray-700">{post.publishedBy}</span>
                      </div>
                    </div>
                  </div>
                </div>
        ...
        <div className="hidden sm:block overflow-x-auto">
          {scheduledPosts.length > 0 ? (
            <table className="w-full table-auto">
              <thead>
                <tr className="border-b-2 border-gray-100 text-sm text-gray-600">
                  <th className="text-left px-4 py-3 font-medium w-1/6">SCHEDULED FOR</th>
                  <th className="text-left px-4 py-3 font-medium w-2/5">POST CONTENT</th>
                  <th className="text-left px-4 py-3 font-medium">PLATFORMS</th>
                  <th className="text-left px-4 py-3 font-medium">SCHEDULED BY</th>
              ...
              <tbody>
                {scheduledPosts.map((post: Post) => (
                  <tr key={post.id} className="border-b border-gray-100 hover:bg-gray-50 text-sm">
                    <td className="px-4 py-4">
                      <div>
                        <div className="text-gray-800 font-medium">{post.scheduledFor}</div>
                        <div className="text-gray-500 text-xs">{post.scheduledTime}</div>
                      </div>
                    </td>
                    <td className="px-4 py-4">
                      <div className="flex items-center">
                    ...
                    <td className="px-4 py-4">
                      <div className="flex items-center">
                        {post.scheduledByAvatar && renderAvatar(post.scheduledByAvatar)}
                        <span className="ml-2 text-gray-800">{post.scheduledBy || 'Owner'}</span>
                      </div>
                    </td>
                ...
                <div key={post.id} className="bg-white rounded-lg shadow-sm overflow-hidden border border-gray-200">
                  <div className="p-4">
                    <div className="flex justify-between items-center mb-3">
                      <div>
                        <div className="text-sm font-medium text-gray-800">{post.scheduledFor}</div>
                        <div className="text-xs text-gray-500">{post.scheduledTime}</div>
                      </div>
                      <ActionButton />
                    </div>
                    ...
                    <div className="flex justify-end items-center pt-2 border-t border-gray-100">
                      <div className="flex items-center">
                        {post.scheduledByAvatar && renderAvatar(post.scheduledByAvatar)}
                        <span className="ml-2 text-xs text-gray-700">{post.scheduledBy || 'Owner'}</span>
                      </div>
                    </div>
                  </div>
                </div>
